# OCI Image: Fedora 43, Integration Test Target (ITT)

[Fedora 43](https://docs.fedoraproject.org/en-US/releases/f43/) (Linux) for integration testing.
...
2. Clone or pull the latest changes from the [`foundata/oci-fedora43-itt` git repository](https://github.com/foundata/oci-fedora43-itt).
3. Change into the directory and execute the [build command](https://docs.podman.io/en/latest/markdown/podman-build.1.html):
   ```bash
   podman build -t fedora43-itt .
   ```
...
2. Use the image you built earlier or pull the image from a registry:
   - [Quay](https://quay.io/repository/foundata/fedora43-itt):
     ```bash
     podman pull quay.io/foundata/fedora43-itt:latest
     ```
   - [Docker Hub](https://hub.docker.com/r/foundata/fedora43-itt):
     ```bash
     podman pull docker.io/foundata/fedora43-itt:latest
     ```
3. Run a container from the image:
   ```bash
   podman run --detach --systemd=always fedora43-itt:latest "/usr/sbin/init"
   ```
...
[![REUSE status](https://api.reuse.software/badge/github.com/foundata/oci-fedora43-itt)](https://api.reuse.software/info/github.com/foundata/oci-fedora43-itt)
